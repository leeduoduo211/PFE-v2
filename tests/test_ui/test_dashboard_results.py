...
def test_wizard_results_tab_uses_dashboard_result_body():
    app_source = (ROOT / "ui" / "app.py").read_text(encoding="utf-8")

    assert "render_dashboard_result_body(latest" in app_source
    assert "render_results_summary(latest)" not in app_source
